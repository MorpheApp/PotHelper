...
public record KeyData(CipherKey value) implements ProtoMessage {
    ...
    public static KeyData parseFrom(ProtoReader reader) throws IOException {
        ...
        while (reader.hasNext()) {
            ...
            if (fieldNumber == 2) {
                value = reader.readOptionalMessage(wireType, CipherKey::parseFrom);
            } else {
    ...
    public void writeTo(ProtoWriter writer) throws IOException {
        if (value != null) {
            writer.writeMessageField(2, value);
        }
    }
}
